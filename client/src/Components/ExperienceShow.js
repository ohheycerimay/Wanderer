...
function ExperienceShow({ user }) {
	...
	function handleDeleteComment(id) {
		const updateCommentArray = experience.posts.filter(
	...
	function updateCommentLikes(post) {
		fetch(`/posts/${post.id}`, {
			...
			.then((r) => r.json())
			.then((freshPost) => {
				const newPostArray = experience.posts.map((p) =>
					p.id !== freshPost.id ? p : freshPost
				);
				...
				<div>
					<div className='image-container'>
						...
						<div className='content-container'>
							...
							<br></br>
							<a className='stat-button' href={experience.link} target='_blank'>
								Click Here to Book
							</a>
						</div>
					</div>
					...
					experience.posts.map((post) => (
						<div className='comment'>
							...
							<button
								...
								onClick={() => updateCommentLikes(post)}
							>
								LIKE
							</button>{' '}
							<button
